...
- 🎯 Entry points to execute your program using `python -m <flask_template>` or `$ flask_template` with basic CLI argument parsing.
...
---
# flask_template Flask Application

Awesome flask_template created by zhuqiyun
...
```bash
git clone https://github.com/zhuqiyun/flask-template flask_template
cd flask_template
make install
```
...
```bash
flask_template create-db   # run once
flask_template populate-db  # run once (optional)
flask_template add-user -u admin -p 1234  # ads a user
flask_template run
```
